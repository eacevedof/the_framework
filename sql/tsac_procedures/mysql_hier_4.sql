...
BEGIN
    ...
    INSERT INTO table_childs VALUES(iIdParent);
    ...
    DO
        ...
        */
        INSERT IGNORE INTO table_childs (id_child)

        SELECT tabhier.id_child
        FROM tabh AS tabhier
        -- Tabla temporal
        INNER JOIN table_childs AS tabtemp 
        ON tabhier.id_parent = tabtemp.id_child
        AND tabhier.id_child NOT IN (SELECT id_child FROM table_childs);
    ...
    SELECT DISTINCT id_child, iIdParent AS id_parent
    FROM table_childs
    ORDER BY 2,1 ASC;
